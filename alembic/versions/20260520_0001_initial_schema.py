...
Revises:
Create Date: 2026-05-20 01:40:00
"""
...
match_status = postgresql.ENUM(
    "waiting", "active", "finished", name="matchstatus", create_type=False
)
player_role = postgresql.ENUM("x", "o", name="playerrole", create_type=False)


def upgrade() -> None:
    bind = op.get_bind()
    postgresql.ENUM(
        "waiting", "active", "finished", name="matchstatus"
    ...
    op.create_table(
        ...
        sa.Column("role", player_role, nullable=True),
        sa.Column("read_at", sa.DateTime(timezone=True), nullable=True),
